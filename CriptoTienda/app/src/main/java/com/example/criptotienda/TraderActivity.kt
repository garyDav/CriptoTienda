...
import com.example.criptotienda.adapter.CryptosAdapter
import com.example.criptotienda.adapter.CryptosAdapterListener
...
import com.example.criptotienda.network.RealtimeDataListener
...
class TraderActivity : AppCompatActivity(), CryptosAdapterListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tvTraderUsername.text = username
    ...
    override fun onBuyCryptoClicked(crypto: Crypto) {
        if (crypto.available > 0) {
            var tmpUser = user?.cryptolist

            if (tmpUser != null) {
                for (userCrypto in tmpUser) {
